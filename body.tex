...
\begin{enumerate}
\item Most scientific use cases should be satisfied by a quantization factor of q=16 or q=32.  This would provide XX, YY, ZZ {\bf (Note this is being addressed now see \#2 in Section~\ref{sec_tbd}.)}

\item The best performaning, off-the-shelf candidate for compression is BZIP2 which acheives a compression factor of 5-7.  Its main drawback is speed but in trading speed for compression factor the use of LZ or ZSTD would roughly double the storage costs.

\item There is no reason to compromise measurements in DRP by compressing products prior to making measurements (i.e. the best possible measurements 
should be provided within data releases).  The caveat to this is that should the pipelines rely upon all PVI images to be computed, measured, and stored
in preparation for a coadd campaign (similar to the current DES pipelines) then the data volume will almost certainly require these intermediate products
be compressed.

\item With the advent of an Alert Processing pipeline, some attention should be paid to determine whether Pre-Covery measurements can meet requirements if lossy-compressed PVIs are used.  A similar test could be used to determine whether compressed templates adversely effect the difference pipeline.
